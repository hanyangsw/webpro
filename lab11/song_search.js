...
function showSongs_JSON(ajax)
{
    ...
    var jtext = JSON.parse(ajax.responseText);
	
    for (var i=0; i<jtext.songs.length; i++) 
	{
	   var title = jtext.songs[i].title;
	   var artist = jtext.songs[i].artist;
	   var genre = jtext.songs[i].genre;
	   var time = jtext.songs[i].time;
       var li = document.createElement("li");
